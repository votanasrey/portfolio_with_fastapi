    ...
    return (
        <header className={`fixed top-0 flex bg-neutral-900/60 justify-between h-20 items-center py-4 px-4 md:px-6 text-white mx-auto lg:px-24 md:py-0 w-full z-30 transition-colors duration-700 ${isScrolled ? 'bg-black/90' : ''}`}>
            <div className='flex flex-row gap-4 items-center'>
                <h1 className='rounded-md text-[1.1rem] lg:text-[1.3rem] hover:bg-sky-700 hover:scale-125 duration-500'>
            ...
            <ul className='hidden lg:flex'>
                {links.map(({ id, link }) => (
                    <li key={id} className='cursor-pointer hover:scale-105 rounded-lg hover:hover:bg-sky-700 p-4 duration-200 hover:text-white text-[1.1rem] lg:text-[1.3rem]'>
